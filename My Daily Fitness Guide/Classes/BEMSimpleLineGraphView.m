...
- (void)layoutSubviews {
    ...
    if ([self.delegate respondsToSelector:@selector(numberOfPointsInLineGraph:)]) {
        ...
        [self printDeprecationWarningForOldMethod:@"numberOfPointsInGraph" andReplacementMethod:@"numberOfvaluesInLineGraph:"];
...
#pragma clang diagnostic pop
    ...
    if ([self.delegate respondsToSelector:@selector(numberOfvaluesInLineGraph:)]) {
        numberOfValues = [self.delegate numberOfvaluesInLineGraph:self];
    }
...
- (void)drawLines {
    CGFloat xDot1 = 0.0; // Postion on the X-axis of the first point.
    CGFloat yDot1 = 0.0; // Postion on the Y-axis of the first point.
    CGFloat xDot2 = 0.0; // Postion on the X-axis of the second point.
    CGFloat yDot2 = 0.0; // Postion on the Y-axis of the second point.
    
    // For Bezier Curved Lines
    CGFloat xDot0 = 0.0; // Postion on the X-axis of the previous point.
    CGFloat yDot0 = 0.0; // Postion on the Y-axis of the previous point.
    CGFloat xDot3 = 0.0; // Postion on the X-axis of the next point.
    CGFloat yDot3 = 0.0; // Postion on the Y-axis of the next point.
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                    xAxisLabel = [self.delegate labelOnXAxisForIndex:(i * numberOfGaps - offset)];
#pragma clang diagnostic pop
                ...
                [labelXAxis setCenter:CGPointMake((self.viewForBaselineLayout.frame.size.width/(numberOfPoints-1))*(i - offset), self.frame.size.height - labelXaxisOffset)];
                //NSLog(@"%f", (self.viewForBaselineLayout.frame.size.width/(numberOfPoints-1))*(i - offset));
...
#pragma clang diagnostic pop
        ...
        [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            //closestDot.alpha = 0; // removed by Yogesh
            self.verticalLine.alpha = 0;
            if (self.enablePopUpReport == YES) {
                self.popUpView.alpha = 0;
                self.popUpLabel.alpha = 0;
...
- (BEMCircle *)closestDotFromVerticalLine:(UIView *)verticalLine {
    ...
    for (BEMCircle *point in self.subviews) {
        if (point.tag > 99 && point.tag < 1000 && [point isKindOfClass:[BEMCircle class]]) {
            //point.alpha = 0; // removed by Yogesh
